...
int main(int argc, char **argv){
   ...
   if(!bouncer) {
	...
	ALLEGRO_FONT *font = al_load_ttf_font("Ubuntu-C.ttf",72,0 );
   ...
   while(1)
   {
      ...
      if(redraw && al_is_event_queue_empty(event_queue)) {
         ...
         float Y = data.posiY;
         al_unlock_mutex(data.mutex);
 			al_clear_to_color(al_map_rgb(X,Y,255-(X+Y)/2));
         al_draw_bitmap(bouncer, X, Y, 0);
		al_draw_text(font, al_map_rgb(255-X,255-Y,X+Y), X/2, (Y/4),ALLEGRO_ALIGN_CENTRE, "Love you ya Vivi!");
