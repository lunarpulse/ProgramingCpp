				...
				reference_wrapper<bool> die)
{
	...
	//{
		...
		cout << dec<< showbase<<"Creating " <<
		numberToCreate <<
		" objects on thread " << hex<< showbase<<
		...
		cout<< dec<< showbase<<++countO<<" MyManagedObjects created"<<endl;
	//}
}
...
int main(int argc, char* argv[])
{
	...
	thread myThread3{ ThreadTask, ref(condition), ref(queueMutex), ref(queue), ref(die) };
	thread myThread4{ ThreadTask, ref(condition), ref(queueMutex), ref(queue), ref(die) };
	 
	 
	queueMutex.lock();
	queue.emplace_back(3000000);
	queue.emplace_back(4000000);
	queue.emplace_back(5000000);
	queue.emplace_back(6000000);
	queue.emplace_back(500000);
